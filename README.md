...
Why did the Ballr gameboard refuse to go on vacation?

Every time it tried to `load()` a new one, it got called right back to `init` — nobody else knows where the entities are supposed to be. 🧳
